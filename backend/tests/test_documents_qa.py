...
from app.models import Lead, Documento, User
...
async def sample_lead(db_session):
    ...
    # Ensure the mock user from conftest exists in DB for Foreign Key constraints
...
async def test_upload_document_success(async_client, sample_lead):
    ...
    with patch("app.services.documento_service.S3StorageAdapter.upload_file", new_callable=AsyncMock) as mock_upload, \
         ...
         patch("app.services.documento_service.send_push_notification", new_callable=AsyncMock) as mock_push:
            ...
            files = {"file": ("passagem.pdf", b"fake pdf content", "application/pdf")}
...
async def test_list_documents_empty(client, valid_jwt_token, sample_lead):
...
async def test_upload_atomic_cleanup_failure(client, valid_jwt_token, sample_lead, mocker):
    """
