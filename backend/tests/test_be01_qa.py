...
async def test_upsert_lead_creation():
    ...
    lead_data = {"telefone": "+5511999999999", "nome": "João Silva"}
...
async def test_database_offline_resilience():
    ...
    db.execute.side_effect = ProgrammingError(
        "statement", "params", 'relation "leads" does not exist'
    )

    with pytest.raises(RuntimeError) as excinfo:
        await lead_service.upsert_lead_with_resilience(
            db, {"telefone": "+5511988888888"}
        )
