...
class AgencySettingsModel(Base):
    """One row per agency_id (singleton today). Stores hours + notification prefs as JSONB."""
    ...
    agency_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), nullable=False, default=SINGLETON_AGENCY_ID
    )
    horario_funcionamento: Mapped[dict] = mapped_column(JSONB, nullable=False)
    notificacoes_prefs: Mapped[dict] = mapped_column(JSONB, nullable=False)
...
class MessageTemplateModel(Base):
    ...
    variaveis: Mapped[list] = mapped_column(JSONB, nullable=False, default=list)
