...
async def get_leads_paginated(
    ...
    # Mapeia leads para dicionários e ajusta o campo score
    items = []
    for lead in leads:
        lead_dict = {
            "id": lead.id,
            "nome": lead.nome,
            "email": getattr(lead, "email", None), # Campo opcional
            "telefone": lead.telefone,
            "status": lead.status,
            "score": float(lead.score_numerico or 0),
            "criado_em": lead.criado_em,
            "atualizado_em": lead.atualizado_em
        }
...
async def mark_stale_leads_as_perdido(db: AsyncSession, inactivity_days: int) -> int:
    """
    Identifica leads inativos (sem atualização há mais de X dias)
    e os move para o status PERDIDO via State Machine.
    """
    limit_date = datetime.now(timezone.utc) - timedelta(days=inactivity_days)
    
    stmt = select(LeadModel).where(
        LeadModel.status != LeadStatus.cliente.value,
        LeadModel.status != LeadStatus.descartado.value,
        LeadModel.atualizado_em < limit_date
    )
    ...
    count = 0
    for lead in leads:
        await update_lead_status(db, lead, LeadStatus.descartado, triggered_by="sistema/rotina_automatica")
        count += 1
